...
from streamvln.model.scene_encoder import SceneEncoder
from streamvln.model.prototype_manager import PrototypeLibrary
from streamvln.model.hyper_lora_layers import replace_linear_with_hyperlora
...
class StreamVLNForCausalLM(Qwen2ForCausalLM, LlavaMetaForCausalLM):
    def __init__(
        ...
        **kwargs,
        ...
        self.use_protostream = getattr(config, 'use_protostream', False)
        
        if self.use_protostream:
            print("[ProtoStream] Initializing ProtoStream components...")
            
            print("[ProtoStream] Freezing base model...")
            ...
            print("[ProtoStream] Base model frozen ✓")
            
            self.scene_encoder = SceneEncoder(
                vision_tower=self.get_vision_tower(),
                text_encoder=None,  
                output_dim=getattr(config, 'proto_dim', 512),
                use_text=False
            )
            
            self.prototype_lib = PrototypeLibrary(
                proto_dim=getattr(config, 'proto_dim', 512),
                similarity_threshold=getattr(config, 'similarity_threshold', 0.7),
                momentum=getattr(config, 'proto_momentum', 0.9),
                max_prototypes=getattr(config, 'max_prototypes', 100),
                diversity_weight=getattr(config, 'diversity_weight', 0.1),
                temperature=getattr(config, 'temperature', 1.0)
            )
            
            target_modules = getattr(config, 'lora_target_modules', ['q_proj', 'v_proj'])
            replace_linear_with_hyperlora(
                model=self.model,
                proto_lib=self.prototype_lib,
            ...
            print(f"[ProtoStream] Trainable params: {trainable_params:,} ({trainable_params/1e6:.2f}M)")
            print(f"[ProtoStream] Total params: {total_params:,} ({total_params/1e6:.2f}M)")
            print(f"[ProtoStream] Trainable ratio: {100*trainable_params/total_params:.4f}%")
            
            print("[ProtoStream] ProtoStream initialization complete!")
    ...
    def encode_rgbd(self, images, depths, poses, intrinsics, time_ids=None, task_ids=None):
        ...
        image_features = vision_tower(images.flatten(0,1))
        
        num_patches_per_side = self.get_model().get_vision_tower().num_patches_per_side
        # (B*V, C, num_patches) -> (B, V, C, H_patch, W_patch)
        ...
        use_protostream = (
            hasattr(self.model, 'scene_encoder') and 
            hasattr(self.model, 'prototype_manager') and
            hasattr(self.model, 'global_hypernet')
        )
        
        if use_protostream:

            scene_features = self.model.scene_encoder(
                image_features,  # [B, V, C, H, W]
                depths,          # [B, V, H_d, W_d]
                poses,           # [B, V, 4, 4]
                intrinsics       # [B, V, 3, 3]
            )  # -> [B, num_views, proto_dim]

            scene_features_pooled = scene_features.mean(dim=1)  # [B, proto_dim]
            
            if self.training:
                proto_indices = self.model.prototype_manager.add_batch(
                    scene_features_pooled
                )
                
                if torch.distributed.is_initialized():
                    rank = torch.distributed.get_rank()
                    if rank == 0:
                        num_protos = len(self.model.prototype_manager.prototypes)
                        print(f"[ProtoStream] Current prototype count: {num_protos}")
            else:
                if len(self.model.prototype_manager.prototypes) > 0:
                    proto_indices, _ = self.model.prototype_manager.match_prototype(
                        scene_features_pooled
                    )  # [B]
                else:
                    proto_indices = torch.zeros(batch_size, dtype=torch.long, device=images.device)
            
            if len(self.model.prototype_manager.prototypes) > 0:
                matched_protos = torch.stack([
                    self.model.prototype_manager.prototypes[idx.item() % len(self.model.prototype_manager.prototypes)]
                    for idx in proto_indices
                ])  # [B, proto_dim]
                
                matched_protos = matched_protos.to(scene_features_pooled.device, scene_features_pooled.dtype)

                lora_weights = self.model.global_hypernet(
                    scene_features_pooled,  
                    matched_protos        
                )  # Dict[layer_name, Dict[weight_name, Tensor]]
                
                from streamvln.model.hyper_lora_layers import HyperLoRALinear
                ...
                for name, module in self.model.named_modules():
                    if isinstance(module, HyperLoRALinear):
                        ...
                        if layer_key in lora_weights:
                            module.set_adapter_weights(lora_weights[layer_key])
        ...
        if num_view != 1:
            ...
            for b in range(batch_size):
                ...
                if his_image_feature.dtype != next(mm_projector.parameters()).dtype:
                    his_image_feature = his_image_feature.to(next(mm_projector.parameters()).dtype)
                
                his_image_feature = mm_projector(his_image_feature)
        ...
        for j, image_feature in enumerate(image_features):
            ...
            mm_projector = self.get_model().mm_projector
            if image_feature.dtype != next(mm_projector.parameters()).dtype:
                image_feature = image_feature.to(next(mm_projector.parameters()).dtype)
            
            image_feature = mm_projector(image_feature)
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.use_protostream and images is not None:
            scene_features = self.scene_encoder(images)
            
            if self.training:
                self.prototype_lib.update_or_create_prototype(scene_features)
            
            if len(self.prototype_lib) > 0:
                _, proto_weights = self.prototype_lib.compute_soft_weights(
                    scene_features,
                    top_k=None
                )
                
                prototypes = self.prototype_lib.get_all_prototypes()  # (K, proto_dim)
                
                weighted_proto = torch.matmul(proto_weights, prototypes)  # (B, proto_dim)
                if weighted_proto.shape[0] == 1:
                    weighted_proto = weighted_proto.squeeze(0)  # (proto_dim,)
                
                self.model.global_hypernet.set_weighted_proto(weighted_proto)
            else:
                self.model.global_hypernet.set_weighted_proto(None)
